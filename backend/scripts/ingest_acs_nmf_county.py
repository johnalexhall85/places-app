...
COLUMN_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "location_id": ("LocationID", "LocationId", "CountyFIPS", "CountyFips"),
    "location_name": ("LocationDesc", "LocationName", "CountyName"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest ACS non-medical factor county estimates into acs_nmf_county_estimates."
    )
    parser.add_argument("--csv", required=True, help="Path to ACS NMF county CSV file.")
    parser.add_argument(
        "--db-url",
        default=None,
    ...
    parser.add_argument(
        "--truncate",
        action="store_true",
        help="Truncate acs_nmf_county_estimates before ingest.",
    )
    parser.add_argument(
        "--chunksize",
        type=int,
...
def prepare_chunk(df: pd.DataFrame) -> pd.DataFrame:
    ...
    valid_required = (
        ...
        & output["location_id"].str.fullmatch(r"\d{5}", na=False)
...
def ensure_target_table_exists(connection) -> None:
    ...
    if row["exists"] is None:
        raise RuntimeError(
            "Table acs_nmf_county_estimates does not exist. Run migrations before ingesting."
        )


def ingest(csv_path: Path, db_url: str, truncate: bool, chunksize: int) -> None:
    engine = create_engine(db_url, future=True)
    staging_table = "tmp_acs_nmf_county_ingest"
    ...
    with engine.begin() as connection:
        ...
        if truncate:
            connection.execute(text("TRUNCATE TABLE acs_nmf_county_estimates"))
    ...
    for raw_chunk in iter_csv_chunks(csv_path, chunksize):
        ...
        with engine.begin() as connection:
            ...
            result = connection.execute(
                text(
                    f"""
                    INSERT INTO acs_nmf_county_estimates (
                        year_window,
                        state_abbr,
                        location_id,
                        location_name,
